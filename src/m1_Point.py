...
class Point(object):
    ...
    def move_to(self, x, y):
        self.delta_d = self.delta_d + self.get_distance_from(Point(x,y))
        self.x = x
        self.y = y
    ...
    def move_by(self,dx,dy):
        self.delta_d = self.get_distance_from(Point(self.x + dx, self.y + dy)) + self.delta_d
        self.x = self.x + dx
        self.y = self.y + dy
        self.move = self.move + 1
    ...
    def get_distance_from_start(self):
        return self.get_distance_from(Point(self.for_x,self.for_y))

    def get_distance_traveled(self):
        return self.delta_d

    def closer_to(self, p2, p3):
        p2_dist = self.get_distance_from(p2)
        p3_dist = self.get_distance_from(p3)
